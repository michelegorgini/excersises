# Michele: I solved Exercise 3 . Great Gratification. The display of the board is very important to understand where
# is the mistake
...
def FourQueens(n):
    ...
    print('Start',*board, sep='\n')
    #Place a queen a column at a time beginning with leftmost column
    for i in range(n):
        board[i][0] = 1
        for j in range(n):
            board[j][1] = 1
            # print('First noConflict',*board, sep='\n')
            if not noConflicts(board, 1, j, n):
                # If this index and the previous are both in 'n-1' steps, I have to re-put 0 also to previous
                # Queen position
                if j == n-1:
                    board[j][1] = 0
                    board[i][0] = 0
            ...
            for k in range(n):
                board[k][2] = 1
                # print('Second noConflict',*board, sep='\n')
                ...
                for m in range(n):
                    board[m][3] = 1
                    # print('Trird noConflict',*board, sep='\n')
                    if noConflicts(board, 3, m, n):
                        print('Final Result',*board, sep='\n')
                        board[m][3] = 0
                    else:
                        if m == k == n-1:
                            board[m][3] = 0
