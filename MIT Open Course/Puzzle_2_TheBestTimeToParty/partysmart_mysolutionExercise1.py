...
def bestTimeToPartySmart(schedule, ystart, yend): # Exercise 1
    #Convert schedule to list of start times and end times marked as such
    times = []
    for c in schedule:
        times.append((c[0], 'start'))
        times.append((c[1], 'end'))    # times.append((c[1]-0.01, 'end')) to avoid 'or' statement in line 52/54
    print(times)
...
##    print times
    ...
    maxcount, time = chooseTime(times, ystart, yend) # Exercise 1
...
def sortlist(tlist):
    ...
    for index in range(len(tlist)-1):
        ...
        #for i in range(ismall+1, len(tlist)):  I think that using 'ismall+1' I can avoid some loop checks
        ...
        tlist[index], tlist[ismall] = tlist[ismall], tlist[index]   # I can do it because the for loops work on the
                                                                    # index not on the element of the array.

    return



def chooseTime(times, ystart, yend): # Exercise 1
    #print(ystart, yend)
    rcount = 0
    maxcount = 0
    time = 0
    ...
    for t in times:
        if t[1] == 'start':
            ...
            rcount = rcount - 1
        if rcount > maxcount and ystart <= t[0] < yend:  # Exercise 1: Solution
            maxcount = rcount
            time = t[0]
